...
!(function($,window,undefined){"use strict";$.fn.tabslet=function(options){var defaults={mouseevent:'click',activeclass:'active',visibleclass:'visible',attribute:'href',animation:!1,autorotate:!1,deeplinking:!1,pauseonhover:!0,delay:2000,active:1,container:!1,controls:{prev:'.prev',next:'.next'}};var options=$.extend(defaults,options);return this.each(function(){var $this=$(this),_cache_li=[],_cache_div=[];var _container=options.container?$(options.container):$this;var _tabs=_container.find('> div');_tabs.each(function(){_cache_div.push($(this).css('display'))});var elements=$this.find('> ul > li'),i=options.active-1;if(!$this.data('tabslet-init')){$this.data('tabslet-init',!0);$this.opts=[];$.map(['mouseevent','activeclass','attribute','animation','autorotate','deeplinking','pauseonhover','delay','container'],function(val,i){$this.opts[val]=$this.data(val)||options[val]});$this.opts.active=$this.opts.deeplinking?deep_link():($this.data('active')||options.active)
_tabs.removeClass($this.opts.visibleclass);if($this.opts.active){_tabs.eq($this.opts.active-1).addClass(options.visibleclass);elements.eq($this.opts.active-1).addClass(options.activeclass)}
var fn=eval(function(e,tab){var _this=tab?elements.find('a['+$this.opts.attribute+'="'+tab+'"]').parent():$(this);_this.trigger('_before');elements.removeClass(options.activeclass);_this.addClass(options.activeclass);if($this.opts.animation){_tabs.hide()}else{_tabs.removeClass(options.visibleclass)}
i=elements.index(_this);var currentTab=tab||_this.find('a').attr($this.opts.attribute);if($this.opts.deeplinking)location.hash=currentTab;if($this.opts.animation){_container.find(currentTab).animate({opacity:'show'},'slow',function(){_this.trigger('_after')})}else{_container.find(currentTab).addClass(options.visibleclass);_this.trigger('_after')}
